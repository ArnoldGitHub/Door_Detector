...
def connect_handler(data):
    # global PUSHER_CHANNEL_NAME, pusher
    logger.log(logging.DEBUG, 'trying to subscribe to: {}'.format(config.get('pusher', 'channel')))

    channel = pusher.subscribe(config.get('pusher', 'channel'))
    channel.bind('status_changed', status_changed_func)
    logger.log(logging.DEBUG, 'Pusher connected, act as receiver')
...
if __name__ == '__main__':     # Program start from here
    ...
    # receiver
    #
    pusher_key = config.get('pusher', 'key')
    pusher_cluster = config.get('pusher', 'cluster')
    ...
    if IS_RASPBERRY_PI:
        ...
        while True:
            ...
            if CONTROL_RANGE_START <= distance <= CONTROL_RANGE_END:
